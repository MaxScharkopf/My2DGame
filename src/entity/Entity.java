...
public class Entity {
    ...
    public int useCost; // cost of shooting projectile
    ...
    public int getParticleSpeed() {
        int speed = 0; // how quickly the particle moves
        return speed;
    }
    public int getParticleMaxLife() {
        int maxLife = 0; // how long the particle lasts
        return maxLife;
    }
    ...
    public void draw(Graphics2D g2) {
        ...
        if(worldX + gp.tileSize > gp.player.worldX - gp.player.screenX && // add gp.tileSize to extend screen render by 1
            ...
            if(type == type_monster && hpBarOn) {
                ...
                g2.setColor(new Color(35,35,35)); // border
                g2.fillRect(screenX - 1,screenY - 16, gp.tileSize+2, 12);

                g2.setColor(new Color(255,0,30)); // health bar
                if(hpBarValue > 0) {
                    g2.fillRect(screenX, screenY - 15, (int) hpBarValue, 10);
                }
                hpBarCounter++;

                // turn off hp bar after 10 seconds of attacking
                if(hpBarCounter > 300) {
                    hpBarCounter = 0;
                    hpBarOn = false;
                }
            ...
            if(invincible) {
                hpBarOn = true;
                hpBarCounter = 0; // ensures while being attacked the bar remains
                changeAlpha(g2, 0.4f);
            }
            if(dying) {
                dyingAnimation(g2);
...
//            if(nextCol == goalCol && nextRow == goalRow) { // disabled bc testing following player
//                onPath = false;
//
//            }
        }
